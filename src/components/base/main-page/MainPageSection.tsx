...
export default function MainPageSection({
  ...
  buttons: { title: string; icon: IconDefinition; variant: string; href: string }[] | undefined;
  screenWidth: number;
  isFlipped: boolean;
}) {
  return (
    <section>
      <Container>
        <Row>
          {isFlipped && screenWidth >= 768 ? (
            <Col style={{ display: 'grid' }} className="align-items-center">
          ...
          <Col xs={12} md={8}>
            <Stack gap={4} className="ms-auto">
              ...
              <Stack
                ...
                direction={screenWidth < 384 ? 'vertical' : 'horizontal'}
                gap={3}
              >
                {buttons === undefined ? (
                  <></>
                ...
                {href === null ? <></> : <ReadMoreButton href={href} />}
              </Stack>
